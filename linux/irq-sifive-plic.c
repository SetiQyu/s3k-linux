...
static void __plic_toggle(void __iomem *enable_base, int hwirq, int enable)
{
	...
	// uint32_t sources = s3k_get_plic_enabled_srcs();
	// uint32_t hwirq_mask = 1 << (hwirq % 32);

	if (enable)
		s3k_plic_toggle(hwirq, 1);
		// writel(readl(reg) | hwirq_mask, reg);
	else
		s3k_plic_toggle(hwirq, 0);
		// writel(readl(reg) & ~hwirq_mask, reg);
}
...
static int plic_irq_suspend(void)
{
	...
	for (i = 1; i < priv->nr_irqs; i++) {
		...
		uint32_t val = s3k_get_irq_priority(i);
...
static void plic_irq_resume(void)
{
	...
	for (i = 1; i < priv->nr_irqs; i++) {
		// index = BIT_WORD(i);
		// Saved previous priority is set 1 again or 0 if nothing there
		// PRIORITY_PER_ID is 32 bits
		s3k_set_irq_prio(i, ((priv->prio_save[index] & BIT_MASK(i)) ? 1 : 0));
	...
	for_each_present_cpu(cpu) {
		...
		for (i = 0; i < DIV_ROUND_UP(priv->nr_irqs, 32); i++) {
			// reg = handler->enable_base + i * sizeof(u32);
			s3k_plic_mask(i, handler->enable_save[i]);
